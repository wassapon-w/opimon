...
for t in range(0, 18):
    for e in range(1, 21):
        for tau in range(1,11):
            if m == 0:
                for tp in range(1, 6):
                    output_results = pd.read_csv("/work/wassapon-w/network_output/"+methods[m]+"/ts_output_day1_"+methods[m]+"_"+target_list[t]+"_E"+str(e)+"_tau"+str(tau)+"_tp"+str(tp)+".csv", header=0, index_col=0)
                    # output_results = pd.read_csv("/work/wassapon-w/network_ddos_output/"+methods[m]+"/ts_ddos_output_day1_"+methods[m]+"_"+target_list[t]+"_E"+str(e)+"_tau"+str(tau)+"_tp"+str(tp)+".csv", header=0, index_col=0)
