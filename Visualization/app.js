...
app.get('/topology', function (req, res) {
  ...
  function getTopology() {
  	...
  	MongoClient.connect(url, function(err, db) {
  		...
  		var cursor = db.collection('topology').find( { timestamp: { $gte: new Date(Date.now() - 30 * 1000), $lt: new Date(Date.now()) } } );
...
app.get('/switch', function (req, res) {
  ...
  function getSwitchPort() {
  	...
  	MongoClient.connect(url, function(err, db) {
  		...
  		var cursor = db.collection('switch_port').find( { timestamp: { $gte: new Date(Date.now() - 30 * 1000), $lt: new Date(Date.now()) } } );
  		cursor.each(function(err, doc) {
  			assert.equal(err, null);
  			if (doc != null) {
